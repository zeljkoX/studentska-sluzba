define(['backbone', 'text!sabloni/studijski-program.html','hogan'], function(Backbone, Templates, Hogan) {
	var StudijskiProgramView = Backbone.View.extend({
		...
		initialize: function() {
			...
			Backbone.trigger('dugme', [{tekst: 'Dodaj Semestar', lokacija: this.lokacija() + 'dodaj-semestar/', klasa: 'btn btn-success', ikona: 'icon-plus-sign icon-white'}]);
			Backbone.trigger('meni',[{tekst: 'Lista Semestra', lokacija: this.lokacija(), aktivan:'true'}]);
